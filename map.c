...
static void draw_clock(Map* M, const int row, const int col);
static void draw_speedboot(Map* M, const int row, const int col);
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case '.':
					...
					break;
                case 'C':
                    draw_clock(M, row, col);
                    break;
                case 'S':
                    draw_speedboot(M, row, col);
                    break;
				default:
					break;
			}
...
static void draw_clock(Map* M, const int row, const int col) {
    al_draw_scaled_bitmap(clock_img, 0, 0, 512, 512, map_offset_x + col * block_width, map_offset_y + row * block_height, block_width, block_height, 0);
}

static void draw_speedboot(Map* M, const int row, const int col) {
    al_draw_scaled_bitmap(speedboot_img, 0, 0, 298, 264, map_offset_x + col * block_width, map_offset_y + row * block_height, block_width, block_height, 0);
}
...
Directions shortest_path_direc_orange(Map* M, Directions dir, int startGridx, int startGridy, int endGridx, int endGridy) {
    ...
    memset(steped, 0, sizeof(steped)); // set as NONE;
    int delta=0;
    while(delta<=4 && !is_wall_block(M, endGridx, endGridy) && !is_room_block(M, endGridx, endGridy)){
        switch (dir){
            case UP:
                delta+=1;
                endGridx -= 1;
